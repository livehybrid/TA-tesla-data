...
class Tesla(requests.Session):
    ...
    def _fetch_jwt(self, oauth):
        ...
        data = {'grant_type': 'urn:ietf:params:oauth:grant-type:jwt-bearer',
                'client_id': CLIENT_ID}
        response = oauth.post(BASE_URL + 'oauth/token', data=data, verify=False, proxies={"http": "http://192.168.0.190:9999", "https": "http://192.168.0.190:9999"})
        response.raise_for_status()  # Raise HTTPError, if one occurred
    ...
    def _token_updater(self):
        ...
        if self.authorized:
            ...
            try:
                ...
                logger.error('Cache not updated - {}'.format(e))
            else:
